...
build-windows: ## Build for Windows
	mkdir -p build
	cd cmd && CC=x86_64-w64-mingw32-gcc CGO_ENABLED=1 GOOS=windows GOARCH=amd64 \
		fyne package -os windows -icon Icon.png \
		-name "$(APP_NAME)_windows_$(shell date +%Y%m%d%H%M%S)" \
		-appID "com.$(APP_NAME)" -release
	mv cmd/$(APP_NAME)_windows_*.exe build/
...
build-macos-m1: ## Build for macOS M1
	...
	cd cmd && GOARCH=arm64 \
		...
		-appID "com.$(APP_NAME)" -release
	mv cmd/$(APP_NAME)_arm64_darwin_*.app build/
...
build-macos-intel: ## Build for macOS Intel
	...
	cd cmd && GOARCH=amd64 \
		...
		-appID "com.$(APP_NAME)" -release
	mv cmd/$(APP_NAME)_amd64_darwin_*.app build/

.PHONY: build-linux
build-linux: ## Build for Linux
	mkdir -p build
	cd cmd && GOOS=linux GOARCH=amd64 \
		fyne package -os linux -icon Icon.png \
		-name "$(APP_NAME)_linux_$(shell date +%Y%m%d%H%M%S)" \
		-appID "com.$(APP_NAME)" -release
	mv cmd/$(APP_NAME)_linux_*.tar.xz build/
